...
interface ISocketActionListener {
    /**
     * Socket通讯IO线程的启动，该方法调用后IO线程将会正常工作。
     * 例如InputStream线程启动后，讲回调此方法，如果 OutPutStream 线程启动，也会回调此方法。
     * 一次成功的双工通讯建立，会调用此方法两次。
     *
     * @param action [IAction.actionReadThreadStart][IAction.actionWriteThreadStart]
     */
    fun onSocketIOThreadStart(action: String)

    /**
     * Socket通讯IO线程的关闭，该方法调用后IO线程将彻底死亡。
     * 例如InputStream线程销毁后，讲回调此方法，如果OutPutStream线程销毁，也会回调此方法。
     * 一次成功的双工通讯销毁，会调用此方法两次。
     *
     * @param action [IAction.actionReadThreadShutdown][IAction.actionWriteThreadShutdown]
     * @param e      线程关闭所遇到的异常信息,正常断开也可能会有异常信息.
     */
    fun onSocketIOThreadShutdown(action: String, e: Exception?)
     ...
     */
    fun onSocketReadResponse(info: ConnectionInfo, action: String, data: OriginalData)
     ...
     */
    fun onSocketWriteResponse(info: ConnectionInfo, action: String, data: ISendable)
     ...
     */
    fun onSocketDisconnection(info: ConnectionInfo, action: String, e: Exception?)
     ...
     */
    fun onSocketConnectionSuccess(info: ConnectionInfo, action: String)
     ...
     */
    fun onSocketConnectionFailed(info: ConnectionInfo, action: String, e: Exception?)
}