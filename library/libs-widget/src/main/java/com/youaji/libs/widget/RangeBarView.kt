 ...
 * @since 2023/1/22
 */
open class RangeBarView @JvmOverloads constructor(
    context: Context,
    ...
    init {
        ...
        if (attrs != null) {
            ...
            val styledAttrs = context.obtainStyledAttributes(attrs, R.styleable.RangeBarView, 0, 0)
            labelColor = styledAttrs.getColor(R.styleable.RangeBarView_labelColor, labelColor)
            activeLabelColor = styledAttrs.getColor(R.styleable.RangeBarView_activeLabelColor, activeLabelColor)
            activeThumbLabelColor = styledAttrs.getColor(R.styleable.RangeBarView_activeThumbLabelColor, activeThumbLabelColor)
            fixedLabelColor = styledAttrs.getColor(R.styleable.RangeBarView_fixedLabelColor, fixedLabelColor)
            fixedThumbLabelColor = styledAttrs.getColor(R.styleable.RangeBarView_fixedThumbLabelColor, fixedThumbLabelColor)

            lineColor = styledAttrs.getColor(R.styleable.RangeBarView_lineColor, lineColor)
            activeLineColor = styledAttrs.getColor(R.styleable.RangeBarView_activeLineColor, activeLineColor)
            fixedLineColor = styledAttrs.getColor(R.styleable.RangeBarView_fixedLineColor, fixedLineColor)
            tickColor = styledAttrs.getColor(R.styleable.RangeBarView_tickColor, tickColor)
            activeTickColor = styledAttrs.getColor(R.styleable.RangeBarView_activeTickColor, activeTickColor)
            fixedTickColor = styledAttrs.getColor(R.styleable.RangeBarView_fixedTickColor, fixedTickColor)
            activeThumbColor = styledAttrs.getColor(R.styleable.RangeBarView_activeThumbColor, activeThumbColor)
            activeFocusThumbColor = styledAttrs.getColor(R.styleable.RangeBarView_activeFocusThumbColor, activeThumbColor)
            fixedThumbColor = styledAttrs.getColor(R.styleable.RangeBarView_fixedThumbColor, fixedThumbColor)

            activeFocusThumbAlpha = styledAttrs.getFloat(R.styleable.RangeBarView_activeFocusThumbAlpha, activeFocusThumbAlpha)

            lineThickness = styledAttrs.getDimension(R.styleable.RangeBarView_lineThickness, lineThickness)
            activeLineThickness = styledAttrs.getDimension(R.styleable.RangeBarView_activeLineThickness, activeLineThickness)
            fixedLineThickness = styledAttrs.getDimension(R.styleable.RangeBarView_fixedLineThickness, fixedLineThickness)
            activeThumbRadius = styledAttrs.getDimension(R.styleable.RangeBarView_activeThumbRadius, activeThumbRadius)
            activeThumbFocusRadius = styledAttrs.getDimension(R.styleable.RangeBarView_activeThumbFocusRadius, activeThumbFocusRadius)
            fixedThumbRadius = styledAttrs.getDimension(R.styleable.RangeBarView_fixedThumbRadius, fixedThumbRadius)
            tickRadius = styledAttrs.getDimension(R.styleable.RangeBarView_tickRadius, tickRadius)
            activeTickRadius = styledAttrs.getDimension(R.styleable.RangeBarView_activeTickRadius, activeTickRadius)
            fixedTickRadius = styledAttrs.getDimension(R.styleable.RangeBarView_fixedTickRadius, fixedTickRadius)
            labelMarginBottom = styledAttrs.getDimension(R.styleable.RangeBarView_labelMarginBottom, labelMarginBottom)
            labelFontSize = styledAttrs.getDimension(R.styleable.RangeBarView_labelFontSize, labelFontSize)
            minDistanceBetweenLabels = styledAttrs.getDimension(R.styleable.RangeBarView_minDistanceBetweenLabels, minDistanceBetweenLabels)
            innerRangePadding = Math.max(calcMaxRadius(), styledAttrs.getDimension(R.styleable.RangeBarView_innerRangePadding, innerRangePadding))
            innerRangePaddingLeft = Math.max(calcMaxRadius(), styledAttrs.getDimension(R.styleable.RangeBarView_innerRangePaddingLeft, innerRangePadding))
            innerRangePaddingRight = Math.max(calcMaxRadius(), styledAttrs.getDimension(R.styleable.RangeBarView_innerRangePaddingRight, innerRangePadding))

            count = styledAttrs.getInt(R.styleable.RangeBarView_count, count)
            startFixed = styledAttrs.getInt(R.styleable.RangeBarView_startFixed, startFixed)
            endFixed = styledAttrs.getInt(R.styleable.RangeBarView_endFixed, endFixed)
            start = styledAttrs.getInt(R.styleable.RangeBarView_start, start)
            end = styledAttrs.getInt(R.styleable.RangeBarView_end, end)
            minDistance = styledAttrs.getInt(R.styleable.RangeBarView_minDistance, minDistance)
            maxDistance = styledAttrs.getInt(R.styleable.RangeBarView_maxDistance, maxDistance)

            movable = styledAttrs.getBoolean(R.styleable.RangeBarView_movable, movable)
            showFixedLine = styledAttrs.getBoolean(R.styleable.RangeBarView_showFixedLine, showFixedLine)
            showTicks = styledAttrs.getBoolean(R.styleable.RangeBarView_showTicks, showTicks)
            showActiveTicks = styledAttrs.getBoolean(R.styleable.RangeBarView_showActiveTicks, showActiveTicks)
            showFixedTicks = styledAttrs.getBoolean(R.styleable.RangeBarView_showFixedTicks, showFixedTicks)
            showLabels = styledAttrs.getBoolean(R.styleable.RangeBarView_showLabels, showLabels)
            isRange = styledAttrs.getBoolean(R.styleable.RangeBarView_isRange, isRange)
    ...
    private fun fadeIn(value: ValueWrapper<Float>, normalValue: Float) = fadeInAnim.apply {
        ...
        addUpdateListener {
            ...
            ViewCompat.postInvalidateOnAnimation(this@RangeBarView)
        }
    ...
    private fun <T> updatePaintsAndRedraw(initialValue: T): ReadWriteProperty<RangeBarView, T> {
    ...
    private fun <T> redraw(
        initialValue: T,
        f: (v: T) -> T = { it },
    ): ReadWriteProperty<RangeBarView, T> {
        return convertable(initialValue, f) { _, _, _ ->
            invalidate()
        }
    }

    private fun <T> updateView(initialValue: T): ReadWriteProperty<RangeBarView, T> {
        return Delegates.observable(initialValue) { _, _, _ ->
            initPaints()
            requestLayout()
        }
    ...
    interface OnChangeRangeListener {
        fun onRangeChanged(rangeView: RangeBarView, start: Int, end: Int)
    }

    interface OnTrackRangeListener {
        fun onStartRangeChanged(rangeView: RangeBarView, leftPinIndex: Int)
        fun onEndRangeChanged(rangeView: RangeBarView, rightPinIndex: Int)
    }

    interface OnRangeLabelsListener {
        fun getLabelTextForPosition(rangeView: RangeBarView, pos: Int, state: State): String?
    }
    ...
    class Builder(private val context: Context) {
        ...
        fun build(): RangeBarView {
            val rangeView = RangeBarView(context)
    ...
    private companion object DefaultValues {
        ...
        val DEFAULT_MINIMAL_DISTANCE_BETWEEN_LABELS = 20f
