...
import com.youaji.libs.debug.widget._float.WARN_ACTIVITY_NULL
import com.youaji.libs.debug.widget._float.anim.AnimatorManager
import com.youaji.libs.debug.widget._float.FloatConfig
import com.youaji.libs.debug.widget._float.FloatDisplayType
import com.youaji.libs.debug.widget._float.interfaces.OnFloatTouchListener
import com.youaji.libs.debug.widget._float.utils.DisplayUtils
import com.youaji.libs.debug.widget._float.utils.InputMethodUtils
import com.youaji.libs.debug.widget._float.utils.LifecycleUtils
import com.youaji.libs.debug.widget._float.utils.Logger
import com.youaji.libs.debug.widget._float.widget.ParentFrameLayout
 ...
 * @since 2024/01/05
 */
internal class FloatWindowHelper(
    val context: Context,
    ...
    fun createWindow(callback: CreateCallback) {
        // 如果在 onCreate 创建单页面浮窗，会存在获取 windowToken 为空的情况，需要异步创建
        if (config.floatDisplayType == FloatDisplayType.OnlyCurrent && getToken() == null) {
    ...
    private fun createWindowInner(): Boolean =
        try {
    ...
    private fun initParams() {
        ...
        params = WindowManager.LayoutParams().apply {
            if (config.floatDisplayType == FloatDisplayType.OnlyCurrent) {
                // 设置窗口类型为应用子窗口，和 PopupWindow 同类型
                type = TYPE_APPLICATION_PANEL
                // 子窗口必须和创建它的 Activity 的 windowToken 绑定
                token = getToken()
            } else {
            ...
            width = if (config.matchWidth) MATCH_PARENT else WRAP_CONTENT
            height = if (config.matchHeight) MATCH_PARENT else WRAP_CONTENT

            if (config.immersionStatusBar && config.matchHeight) {
                height = DisplayUtils.getScreenHeight(context)
            }

            // 如若设置了固定坐标，直接定位
            if (config.locationPoint != Point(0, 0)) {
                x = config.locationPoint.x
                y = config.locationPoint.y
            }
        }
    }
    ...
    private fun addView() {
        ...
        val floatingView =
            config.layoutView?.also { frameLayout?.addView(it) }
        ...
        frameLayout?.layoutListener = object : ParentFrameLayout.OnLayoutListener {
            override fun onLayout() {
                ...
                config.apply {
                    ...
                    if (filterSelf
                        || (floatDisplayType == FloatDisplayType.OnlyBackground && LifecycleUtils.isForeground())
                        || (floatDisplayType == FloatDisplayType.OnlyForeground && !LifecycleUtils.isForeground())
                    ) {
    ...
    private fun setGravity(view: View?) {
        if (config.locationPoint != Point(0, 0) || view == null) return
        val parentRect = Rect()
        // 获取浮窗所在的矩形
        ...
        // 设置偏移量
        params.x += config.offsetPoint.x
        params.y += config.offsetPoint.y
    ...
    private fun enterAnim(floatingView: View) {
        if (frameLayout == null || config.isAnimating) return
     ...
     */
    fun exitAnim() {
        if (frameLayout == null || (config.isAnimating && enterAnimator == null)) return
    ...
    fun remove(force: Boolean = false) = try {
        config.isAnimating = false
        FloatWindowManager.remove(config.floatTag)
